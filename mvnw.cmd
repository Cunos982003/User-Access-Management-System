...
IF NOT EXIST "%MAVEN_PROJECTBASEDIR%\.mvn\wrapper\maven-wrapper.jar" goto wrapperNotFound
...
if exist %MAVEN_WRAPPER_JAR% (
    ...
    powershell -Command "&{"^
		...
		"}"
    if "%MVNW_VERBOSE%" == "true" (
        echo Finished downloading %MAVEN_WRAPPER_JAR%
    )
)
...
IF NOT %WRAPPER_SHA_256_SUM%=="" (
    powershell -Command "&{"^
       "$hash = (Get-FileHash \"%MAVEN_WRAPPER_JAR%\" -Algorithm SHA256).Hash.ToLower();"^
       "If('%WRAPPER_SHA_256_SUM%' -ne $hash){"^
       "  Write-Output 'Error: Failed to validate Maven wrapper SHA-256, your Maven wrapper might be compromised.';"^
       "  Write-Output 'Investigate or delete %MAVEN_WRAPPER_JAR% to attempt a clean download.';"^
